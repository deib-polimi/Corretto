...
public class MadesModel {
	...
	public MadesModel(org.eclipse.uml2.uml.Model m) {
		...
		try {
			...
			if (this.system_package == null) {
				throw new Exception(
						"System package not found: unable to translate the model");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Set<SequenceDiagram> getSequenceDiagrams() {
		...
		for (Element e : this.system_package.getOwnedElements()) {
			if (e instanceof org.eclipse.uml2.uml.Interaction
					&& !UML2ModelHelper.hasStereotype(e, "Ignore")) {
				sequencediagrams.add(new SequenceDiagram(
						(org.eclipse.uml2.uml.Interaction) e));
			}
		}
	...
	public SequenceDiagram findSequenceDiagram(String name) {
		for (Element e : this.system_package.getOwnedElements()) {
			if (e instanceof org.eclipse.uml2.uml.Interaction
					&& ((org.eclipse.uml2.uml.Interaction) e).getName().equals(
							name)
					&& !UML2ModelHelper.hasStereotype(e, "Ignore")) {
